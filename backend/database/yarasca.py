...
class Parser:
    ...
    def parse_column_definition(self) -> CreateColumnDefinition:
        ...
        if column_type not in (ColumnType.INT, ColumnType.FLOAT, ColumnType.VARCHAR, ColumnType.DATE, ColumnType.BOOL):
            raise SyntaxError(f"Invalid column type {self.prev.text} for column {column_name}")
    ...
    def parse_insert_statement(self) -> InsertStatement:
        raise NotImplementedError("Insert statement parsing is not implemented yet")
    ...
    def parse_statement(self) -> Statement:
        if self.match(TokenType.CREATE):
            if self.match(TokenType.TABLE):
                ...
                raise SyntaxError(f"Expected TABLE or INDEX after CREATE, found {self.curr.text}")
