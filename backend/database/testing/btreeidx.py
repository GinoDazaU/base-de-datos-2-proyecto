...
def _test_btreeidx(n: int):
    ...
    print(f"Tabla '{table_name}' creada con índice btree en 'nombre'.")

    nombre_objetivo = None

    for ext in [".hash.idx", ".hash.tree", ".hash.db"]:
        path = f"{table_name}.nombre{ext}"
    ...
    t4 = time.time()
    print(f"Con índice btree: {len(res_btree_idx)} encontrado(s) en {t4 - t3:.6f} s")
    ...
    print(f"Con índice btree: {'ENCONTRADO' if res2 else 'NO ENCONTRADO'}")
